...
**Build:** `npm run build` → output **`dist`** (not `nuxt generate`). [`wrangler.toml`](wrangler.toml) sets `nodejs_compat` + `pages_build_output_dir`.

**Dashboard checklist:** [docs/CLOUDFLARE_DASHBOARD.md](docs/CLOUDFLARE_DASHBOARD.md) — Preview (`*.pages.dev`) vs Production (custom domain) env vars.
...
**Smoke (live):** `GET https://YOUR-PROJECT.pages.dev/api/minisite/sessions` → JSON or 502 (not HTML 404).
